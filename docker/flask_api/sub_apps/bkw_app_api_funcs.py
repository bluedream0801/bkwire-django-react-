...
def list_bankruptcies(guid, **kwargs):
    ...
    if 'search' in kwargs:
        if len(kwargs['search']) > 0:
            return (run_sql_queries_no_cache_v4(output, kwargs['search'][0], fix_query_count))
        else:
...
def list_losses(guid, **kwargs):
    ...
    if 'search' in kwargs:
        if len(kwargs['search']) > 0:
            return (run_sql_queries_no_cache_v3(output, kwargs['search'][0], fix_query_count))
        else:
            return (run_sql_queries_no_cache(output, fix_query_count))
    else:
        return (run_sql_queries_no_cache(output, fix_query_count))
...
def case_existing_check(pacer_data_results):
    ...
    for key in pacer_data_results:
        case_name = key
        case_num = pacer_data_results[key]['case_number']

        sql_query = f"""SELECT * FROM bankruptcy_filing_data WHERE case_name = '{case_name}' AND case_number = '{case_num}'"""
        sqlq_results = BkDbFunc.fetch_no_cache(sql_query)
...
def parse_pacer_results(p1, p2, pacer_data_results, case_refresh_trigger=False):
    ...
    pacer_data_results = case_existing_check(pacer_data_results)
    ...
    if not connection is None:
        ...
        for k in pacer_data_results:
            notify_docket_updates(pacer_data_results[k]['dci_id'])
                ...
                if kv.strip() in data_204206_list_keys:
                    try:
                        ...
                        if len(comp_name_fixed) > 0:
                            ...
                            company_name = comp_name_fixed[0].strip()
                            comp_scrub = company_scrub(company_name)

                            comp_title_comp = title(comp_scrub)
                                           
                            insert_companies(connection, comp_title_comp, data_204206_results[k][kv]['industry'])
                            insert_data_b204206d(connection, comp_title_comp, data_204206_results[k][kv]['creditor_company_zip'], data_204206_results[k][kv]['nature_of_claim'].upper(), data_204206_results[k][kv]['unsecured_claim_value'], data_204206_results[k][kv]['industry'], pacer_data_results[k]['bkw_filing_id'])
                        else:

                            company_name = data_204206_results[k][kv]['creditor_company_name'].strip()
                            comp_scrub = company_scrub(company_name)
                    ...
                    try:
                        ...
                        if len(data_204206_results[k][kv]['creditor_phone']) > 0:
                            ...
                            if len(comp_name_fixed) > 0:
                                insert_data_additional_info(connection, comp_title_comp, joined_string_email, joined_string_phone, pacer_data_results[k]['bkw_filing_id'])
                            else:
                                comp_name_fixed = data_204206_results[k][kv]['creditor_company_name'].strip()
...
def run_sql_queries_no_cache_v4(query, value, my_query_count=None):
    #logger.debug(f"{query}")
    db1 = dbSelectItems()
    db1.db_login()
    if db1.db_cnxn != None:
        if my_query_count == None:
            ...
            my_query_result = db1.fetch_execute_v2(query, value)
            my_query_result_count = db1.fetch_execute_v2(my_query_count, value)
            page_info = {
                  'records': my_query_result, # all the response items will go in this array
                  'count': my_query_result_count[0]['result_count'], # current page
            }
